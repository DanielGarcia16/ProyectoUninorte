    ...
    try:
        ...
        if request.method == 'POST': 
            ...
            if attempt is not None:
                ...
                db.execute(
                    'SELECT username,password,salt,email FROM user where id', (attempt['username'], attempt['password'], attempt['salt'], attempt['email'])
                )
                db.commit()
...
def register():
    try:
        ...
        if request.method ==  'POST':    
            ...
            if db.execute('SELECT id FROM user WHERE usarname = ?', (username,)).fetchone() is not None:
...
def change():
    try:
        ...
        if request.method == 'POST': 
            ...
            db = get_db() if not g.db else g.dbc
            attempt = db.execute(
                'SELECT password,id FROM user WHERE email = ?', (number, utils.F_ACTIVE)
            ).fetchone()
...
def forgot():
    try:
        ...
        if request.method == 'POST':
            ...
            db = get_db()
            user = db.execute(
                'Select username from user when email', (email,)
            ).fetchone()
